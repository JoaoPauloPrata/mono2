...
class FinalResultSplitter:
    ...
    def _assign_groups(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Atribui grupos por empate estatístico usando sobreposição de IC:
        - Ordena por média decrescente.
        - Se o IC do algoritmo atual sobrepõe qualquer IC do grupo corrente,
          permanece no grupo; caso contrário, abre novo grupo.
        """
        if df.empty:
            return df
        ...
        groups = []
        current_group = 1
        # mantém lista de ICs do grupo corrente para checar sobreposição
        current_cis = []
