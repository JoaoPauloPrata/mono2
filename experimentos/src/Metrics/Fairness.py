...
class Fairness:
    ...
    def evaluateAllMetricsForAllMethods(self, window_count, top_n=None):
        ...
        user_informations = pd.read_csv(f"./data/ml-1m/users.dat", sep="::", header=None)
        user_informations = user_informations.rename(columns={0: "user", 1: "gender", 2: "age", 3: "occupation", 4: "zip_code"})
        user_informations = user_informations[["user", "gender"]]
        male_users = user_informations[user_informations["gender"] == "M"]
        female_users = user_informations[user_informations["gender"] == "F"]

        # Carrega ground truth
        truth_df = pd.read_csv(f"./data/windows/test_to_get_constituent_methods_{window_count}.csv")
        truth_male_df = truth_df[truth_df["user"].isin(male_users["user"])]
        truth_female_df = truth_df[truth_df["user"].isin(female_users["user"])]

        truth_male_df = truth_male_df.rename(columns={"rating": "true_value"})
        truth_female_df = truth_female_df.rename(columns={"rating": "true_value"})
        ...
        def compute_metrics_from_frames(pred_df: pd.DataFrame, truth_subset: pd.DataFrame, method_name: str, results: list):
        ...
        for constituent in constituent_algorithms:
            ...
            compute_metrics_from_frames(recs_df, truth_male_df, constituent, results_male)
            compute_metrics_from_frames(recs_df, truth_female_df, constituent, results_female)
        ...
        for hybrid in hybrid_algorithms:
            ...
            compute_metrics_from_frames(pred_df, truth_male_df, hybrid, results_male)
            compute_metrics_from_frames(pred_df, truth_female_df, hybrid, results_female)

        # Cria diretórios para salvar resultados
        os.makedirs('./data/MetricsForMethods/Fairness', exist_ok=True)
        
        # Salva resultados masculinos
        out_male_path = f"./data/MetricsForMethods/Fairness/MetricsForWindow{window_count}Male.csv"
        if results_male:
            out_male_df = pd.DataFrame(results_male).set_index('method')        
            out_male_df = out_male_df.loc[[*constituent_algorithms, *hybrid_algorithms]]
            out_male_df.to_csv(out_male_path)
            print(f"Métricas masculinas salvas em: {out_male_path}")
        
        # Salva resultados femininos
        out_female_path = f"./data/MetricsForMethods/Fairness/MetricsForWindow{window_count}Female.csv"
        if results_female:
            out_female_df = pd.DataFrame(results_female).set_index('method')        
            out_female_df = out_female_df.loc[[*constituent_algorithms, *hybrid_algorithms]]
            out_female_df.to_csv(out_female_path)
            print(f"Métricas femininas salvas em: {out_female_path}")
        
        print(f"Avaliação de fairness concluída para janela {window_count}")
